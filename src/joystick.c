...
const char joystick_cfg_version = 0;
const int joystick_analog_max = 32767;
...
void poll_joystick( int j )
{
	...
	bool pressable = joystick[j].joystick_delay < SDL_GetTicks();
	...
	for (int d = 0; d < COUNTOF(joystick[j].direction); d++)
	{
		...
		joystick[j].direction_pressed[d] &= (!old || pressable);
	...
	for (int d = 0; d < COUNTOF(joystick[j].action); d++)
	{
		...
		joystick[j].action_pressed[d] &= (!old || pressable);
	...
	if (joystick[j].input_pressed) {
		joystick[j].joystick_delay = SDL_GetTicks() + joystick_delay_reset;
	}
}
...
bool detect_joystick_assignment( int j, struct joystick_assignment_struct *assignment )
{
	...
	Sint16 *axis = malloc(axes * sizeof(*button));
